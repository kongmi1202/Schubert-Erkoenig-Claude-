function buildFallbackEssay(data) {
  ...
  const songTitle = isHandel ? "헨델의 <할렐루야>" : "슈베르트의 <마왕>";
  ...
  const chars = isHandel
    ...
    : (data.analyticalCharacters?.filter(Boolean).join(', ') || '해설자, 아버지, 아들, 마왕');
  const sensory = data.sensoryDesc?.trim() || (isHandel ? '음악은 밝고 장엄한 분위기를 강하게 전달했다.' : '음악은 긴장감과 어두운 분위기를 강하게 전달했다.');
  const story = isHandel
    ? (data.handelOperaDiff?.trim() || '오페라와 달리 연기 없이 합창과 관현악으로 종교적 내용을 전하는 오라토리오의 특징을 확인했다.')
    : (data.analyticalStory?.trim() || '폭풍우 치는 밤, 아버지가 아들을 안고 달리는 비극적 이야기로 이해했다.');
...
분석 단계에서 ${isHandel ? `가사와 작품 형식을 ${chars}, ${story}로 정리했다.` : `이 곡의 줄거리는 ${story}이며, 주요 등장인물은 ${chars}라고 정리했다.`} ${isHandel ? '음화법과 가락선, 역사적 맥락을 함께 살펴보니' : '인물의 음색과 피아노 반주를 분석해 보니'} 처음에는 막연했던 느낌이 ${q1}는 생각으로 바뀌었다.
...
- 입력에 없는 사실은 만들지 말 것.
- selectedSong이 handel이면 헨델 할렐루야 기준으로, 아니면 슈베르트 마왕 기준으로 작성할 것.
