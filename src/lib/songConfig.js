...
const LYRICS = {
  mawang: `[🎙️해설자] 누가 늦은 밤 말을 달려 그들은 아버지와 아들 / 아버지 아이를 품에 안고, 안고 달리네, 따뜻하게.
[👨아버지] 아들아, 무엇 때문에 떠느냐?
...
할렐루야!`,
  schoenberg: `Den Wein, den man mit Augen trinkt,
눈으로 마시는 포도주를
...
Den Wein, den man mit Augen trinkt.
눈으로 마시는 포도주를.`,
  haydn: `1) 시작 주제: 가장 먼저 들리는 선율이 어떤 악기인지 짐작해보세요.
...
4) 전체 감상: "종달새"라는 제목과 연결되는 이유를 선율의 움직임으로 설명해보세요.`,
  chopin: `1) 시작(빠른 구간): 오른손 선율과 왼손 반주의 속도 차이를 먼저 들어보세요.
...
export function getSongListeningContent(selectedSong) {
  const config = SONG_CONFIG[selectedSong] || SONG_CONFIG.mawang;
  const songKey = SONG_CONFIG[selectedSong] ? selectedSong : 'mawang';
  return {
    ...config,
    songKey,
    panelLabel: config.panelLabel || '가사',
    lyricsText: LYRICS[songKey] || LYRICS.mawang
  };
}
